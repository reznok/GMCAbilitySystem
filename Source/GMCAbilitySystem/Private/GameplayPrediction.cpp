...
FPredictionKey FPredictionKey::CreateNewPredictionKey(UGMC_AbilitySystemComponent* OwningComponent)
{
...
FPredictionKey FPredictionKey::CreateNewServerInitiatedKey(UGMC_AbilitySystemComponent* OwningComponent)
{
	FPredictionKey NewKey;
...
FScopedPredictionWindow::FScopedPredictionWindow(UGMC_AbilitySystemComponent* AbilitySystemComponent, FPredictionKey InPredictionKey, bool InSetReplicatedPredictionKey /*=true*/)
{
...
FScopedPredictionWindow::FScopedPredictionWindow(UGMC_AbilitySystemComponent* InAbilitySystemComponent, bool bCanGenerateNewKey)
{
	...
	{
		UE_LOG(LogTemp,Warning, TEXT("FScopedPredictionWindow() aborting due to Owner (ASC) being null, destroyed or pending kill / unreachable"));
		return;
	}
...
{
	if (UGMC_AbilitySystemComponent* OwnerPtr = Owner.Get())
	{
...
{
	UE_LOG(LogGMCAbilitySystem,Log, TEXT("FReplicatedPredictionKeyItem::OnRep %s"), *PredictionKey.ToString());
		...
		{	
			...
			if (MapIt.Key() % FReplicatedPredictionKeyMap::KeyRingBufferSize == Index)
			{
				// Message the log, this can happen during normal gameplay due to replication order, but can also indicate an ability-specific issue
					UE_LOG(LogGMCAbilitySystem,Log, TEXT("Passed PredictionKey %d in Delegate map while OnRep'ing %s"), MapIt.Key(), *PredictionKey.ToString());
