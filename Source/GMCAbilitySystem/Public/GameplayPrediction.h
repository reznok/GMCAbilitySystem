 ...
 *	-AbilityEffect removal
 *	-AbilityEffect periodic effects (dots ticking)
 *	
 *	
 ...
 *	4. Server decides if the ability really happened in ServerTryActivateAbility, calls ClientActivateAbility(Failed/Succeed) and sets UGMC_AbilitySystemComponent::ReplicatedPredictionKey to the generated key that was sent.
 ...
 *		ThisPredictionKey.NewRejectedDelegate().BindUObject(this, &UGMC_AbilitySystemComponent::OnClientActivateAbilityFailed, Handle, ThisPredictionKey.Current);
 *		
 *		
 ...
 *		key used previous steps, the client can undo its predictive side effects. See UGMC_AbilitySystemComponent::OnRep_PredictionKey.
 *						
 *			 
 *			 
 *			 
 *	*** AbilityEffect Prediction ***
 *
 *	AbilityEffects are considered side effects of prediction and are not explicitly asked about.
 *	
 *	1. AbilityEffects are only applied on clients if there is a valid prediction key. (If no prediction key, it simply skips the application on client).
 *	2. Attributes, GameplayCues, and GameplayTags are all predicted if the AbilityEffect is predicted.
 *	3. When the FActiveAbilityEffect is created, it stores the prediction key (FActiveAbilityEffect::PredictionKey)
 *		3a. Instant effects are explained below in "Attribute Prediction".
 *	4. On the server, the same prediction key is also set on the server's FActiveAbilityEffect that will be replicated down.
 *	5. As a client, if you get a replicated FActiveAbilityEffect with a valid prediction key on it, you check to see if you have an ActiveAbilityEffect with that same key, if there is match, we do not apply
 *		the 'on applied' type of logic, e.g., GameplayCues. The solves the "Redo" problem. However we will have 2 of the 'same' AbilityEffects in our ActiveAbilityEffects container, temporarily:
 *	6. At the same time, UGMC_AbilitySystemComponent::ReplicatedPredictionKey will catch up and the predictive effects will be removed. When they are removed in this case, we again check PredicitonKey and decide 
 ...
 *	
 *	
 *	1. We treat predictive instant gameplay effects as infinite duration gamepaly effects. See UGMC_AbilitySystemComponent::ApplyAbilityEffectSpecToSelf.
 ...
 *	3. In the attribute RepNotify, we call into the AbilitySystemComponent::ActiveAbilityEffects to update our 'final value' give the new 'base value'. the GAMEPLAYATTRIBUTE_REPNOTIFY can do this.
 *	4. Everything else will work like above (AbilityEffect prediction) : when the prediction key is caught up, the predictive AbilityEffect is removed and we will return to the server given value.
 *	
 *	
 *	Example:
 *	
 ...
 *  *** Gameplay Cue Events ***
 *  
 *  Outside of AbilityEffects which are already explained, Gameplay Cues can be activated on their own. These functions (UGMC_AbilitySystemComponent::ExecuteGameplayCue etc)  take network role and prediction keys into account.
 *  
 *  1. In UGMC_AbilitySystemComponent::ExecuteGameplayCue, if authority then do the multicast event (with replication key). If non authority but w/ a valid prediction key, predict the GameplayCue.
 ...
 *	is not logically atomic. Each side effect produced in the chain of events has to have a unique PredictionKey. We cannot have AbilityEffects produced in X have the same PredictionKey produced in Z.
 *	
 ...
 *	3. Server runs ServerInputRelease_Implementation which takes the passed in PredictionKey and sets it as UGMC_AbilitySystemComponent::ScopedPredictionKey with an FScopedPredictionWindow.
 *	4. Server runs UAbilityTask_WaitInputRelease::OnReleaseCallback /within the same scope/
 *	5. When the server hits the FScopedPredictionWindow in ::OnReleaseCallback, it gets the prediction key from UGMC_AbilitySystemComponent::ScopedPredictionKey. That is now used for all side effects within this logical scope.
 ...
 *	for this should eventually be added and it should follow the same pattern that AbilityEffect and GameplayCues follow (predict triggered event with a prediction key, ignore the RPC event if it has a prediction key).
 *	
 *		 
 *	*** Predicting "Meta" Attributes such as Damage/Healing vs "real" attributes such as Health ***
 *	
 *	We are unable to apply meta attributes predictively. Meta attributes only work on instant effects, in the back end of AbilityEffect (Pre/Post Modify Attribute on the UAttributeSet). These events are not called when
 *	applying duration-based gameplay effects. E.g., a AbilityEffect that modifies damage for 5 seconds doesn't make sense. 
 *	
 *	In order to support this, we would probably add some limited support for duration based meta attributes, and move the transform of the instant gameplay effect from the front end (UGMC_AbilitySystemComponent::ApplyAbilityEffectSpecToSelf)
 *	to the backend (UAttributeSet::PostModifyAttribute). 
 * 
 *			 
 *	*** Predicting ongoing multiplicitive AbilityEffects ***
 *	
 ...
 *	receives a AbilityEffect that slows them and their material blue. Since we can't send Server RPCs every time this happens (and the server couldn't necessarily handle the message at its point in the simulation), there is no 
 ...
 *	done on a client. UGMC_AbilitySystemComponent supports synchronization of the prediction key and its side effects
 ...
 *	
 *	While the client is predicting its ability, it is creating side effects (AbilityEffects, TriggeredEvents, Animations, etc).
...
USTRUCT()
struct GMCABILITYSYSTEM_API FPredictionKey
{
	...
	static FPredictionKey CreateNewPredictionKey(class UGMC_AbilitySystemComponent*);

	/** Construct a new server initiation key, for abilities activated on the server */
	static FPredictionKey CreateNewServerInitiatedKey(class UGMC_AbilitySystemComponent*);
...
struct GMCABILITYSYSTEM_API FScopedPredictionWindow
{
	 ...
	 **/
	FScopedPredictionWindow(UGMC_AbilitySystemComponent* AbilitySystemComponent, FPredictionKey InPredictionKey, bool InSetReplicatedPredictionKey = true);
	...
	FScopedPredictionWindow(UGMC_AbilitySystemComponent* AbilitySystemComponent, bool CanGenerateNewKey=true);

	~FScopedPredictionWindow();

private:

	TWeakObjectPtr<UGMC_AbilitySystemComponent> Owner;
